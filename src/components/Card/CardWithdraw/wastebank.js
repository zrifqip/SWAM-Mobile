...
function CardWithdrawWasteBank({ 
    item,
    onPress
}) {
    return (
        <TouchableOpacity style={styles.cardMenu} activeOpacity={0.5} onPress={onPress}>
            <View style={StC.flexR}>
                <Text style={styles.textName} numberOfLines={1}>{item.customer?.fullName}</Text>
                <Text style={[styles.textType, {backgroundColor: item.status == 'pending' ? Colors.THIRD : Colors.SECONDARY}]} numberOfLines={1}>{item.status}</Text>
            </View>
