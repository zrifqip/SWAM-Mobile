...
function CardChats({ 
    ...
    return (
        <TouchableOpacity style={styles.cardMenu} activeOpacity={0.5} onPress={onPress}>
            ...
            <View style={{flex: 1}}>
                <Text style={styles.textDay} numberOfLines={1}>{respon.name}</Text>
                <View style={[StC.flexR, StC.mT5, {alignItems:'flex-end'}]}>
                    <Text style={styles.textMessage} numberOfLines={1}>{item.lastMsg?.msg}</Text>
                    <Text style={styles.textTime}>{formatDateChats(item.lastMsg?.datetime)}</Text>
                </View>
            </View>
        </TouchableOpacity>
    )
