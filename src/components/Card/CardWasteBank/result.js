                    ...
                    <View style={{flex:1}}>
                        <View style={[StC.flexR, StC.mT10]}>
                            {serviceWasteBank(item?.companyService).map((service) => (
                                <Text style={[styles.textServices, {backgroundColor: service.color}]}>{service.name}</Text>
                            ))}
                        </View>
                        <Text style={styles.km}>{numberFloat(item.distance.toFixed(2))} km</Text>
                    </View>
                    <MyView style={{justifyContent:'flex-end', alignItems:'flex-end'}} hide={parseFloat(item.distance) < 5}>
