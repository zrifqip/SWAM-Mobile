...
function ModalWithdraw({ 
    ...
    const [amount, setAmount]           = useState(user.role == 'bank-sampah' ? user.organization.minimumWithdrawal : '');
    ...
    const sendNotif = async () => {
        ...
        let message = '*## NEW WITHDRAW ##*' + '\n\n' +
        'Hai *' + user.biodata.company.companyName + '*,' + '\n\n' +
        'Berikut invoice untuk pengajuan withdraw' + '\n\n' +
        'Nama Nasabah : ' + user.biodata.fullName + '\n' + 
        'Alamat : ' + user.biodata.address.street + '\n' +
        'Nominal : *' + currencyFloat(amount) + '*\n\n' + 
    ...
    return (
        ...
        >
            <View style={styles.modal}>
                <Text style={styles.balance}>{translations["balance"]}: {currencyFloat(balance)}</Text>
                <Text style={styles.service}>Nominal {translations["withdraw"]} {requireds('*')}</Text>
                ...
                <Text style={styles.min}>* Minimum {translations["withdraw"]} : {currencyFloat(user?.biodata?.company?.minimumWithdrawal)}</Text>
                <View style={StC.mT10}>
                    <Text style={StC.title}>Metode Penarikan {requireds('*')}</Text>
                    <View style={StC.flexR}>
                        <FormInputRadio onPress={() => setIsTransfer(!isTransfer)} checked={!isTransfer} title={'Cash'} style={{width: RFValue(90)}}/>
                        <FormInputRadio onPress={() => setIsTransfer(!isTransfer)} checked={isTransfer} title={'Transfer'} style={{width: RFValue(90)}}/>
                    </View>
                    <View style={StC.mB10}/>
                </View>
                {isTransfer && 
                    <View>
                        {bank ?
                        <Text style={styles.bank}>{bank.name} - {bank.accountName} ({bank.accountNumber})</Text> : 
                        <Text style={[styles.bank, Font.DANGER]}>Atur Bank Transfer terlebih dahulu</Text>}
                    </View>
                }
                <ButtonFlex
