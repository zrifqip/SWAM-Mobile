    ...
    return (
        ...
        >
            <View style={styles.modal}>
                <Text style={StC.title}>{translations["waste.type"]} {requireds('*')}</Text> 
                <View style={StC.mB10}>
                    <FlatList
                        ...
                        renderItem={(({ item, index }) => (
                            <TouchableOpacity style={[styles.cardItem, type == item && {borderColor: Colors.PRIMARY, backgroundColor: Colors.PRIMARY}]} activeOpacity={0.5} onPress={()=> setType(item)}>
                                <Text style={[styles.txtItem, type == item && Font.WHITE]}>{item?.name}</Text>
                            </TouchableOpacity>
                        ))}
                    />
                </View>
                ...
                <View style={styles.cardPrice}>
                    <Text style={styles.labelPrice}>{translations["estimate.price"]}</Text>
                    <Text style={styles.price}>{currencyFloat(type.purchasePrice)} / kg</Text>
                </View>
                <View style={styles.cardPrice}>
                    <Text style={styles.labelPrice}>Sub Total</Text>
                    <Text style={styles.price}>{currencyFloat(weight * type.purchasePrice)}</Text>
                </View>
                <ButtonFlex
