...
function ModalWithdrawDetail({ 
    ...
    const confirmationAccept = (type) => {
        Alert.alert(
            ...
            type == 'reject' ? translations["confirm.reject"] : translations["confirmation.approved"],
            [
                {
    ...
    const sendNotif = async (type) => {
        ...
        let message = 'Hai *' + customer.fullName + '*,' + '\n\n' +
        'Pengajuan withdraw kamu sebesar *' + currencyFloat(item.nominal) + '* ditolak oleh *' + users.users.organization.companyName + '*,' + '\n\n' +
        ...
        if(type == 'accept'){
            message = 'Hai *' + customer.fullName + '*,' + '\n\n' +
            'Pengajuan withdraw kamu sebesar *' + currencyFloat(item.nominal) + '* disetujui oleh *' + users.users.organization.companyName + '*,' + '\n\n' +
    ...
    return (
        ...
        >
            ...
            <View style={styles.modal}>
                ...
                <Text style={styles.balance}>{translations["balance"]}: {currencyFloat(customer.balance)}</Text>
                <Text style={styles.nominal}>Nominal {translations["withdraw"]}: {currencyFloat(item.nominal)}</Text>
                <Text style={styles.nominal}>{item.method == 'cash' ? 'Cash' : 'Transfer'}</Text>
                {item.method == 'transfer' &&
                    <Text style={styles.nominal}>{bank.name} - {bank.accountName} ({bank.accountNumber})</Text>
                }
                <View style={[StC.flexR, StC.mT15, {marginHorizontal: RFValue(-10)}]}>
                    {item.status == 'pending' &&
                        <>
                            <TouchableOpacity activeOpacity={0.5} onPress={()=> confirmationAccept('reject')} style={[styles.btn, styles.btnReject]}>
                                <Text style={styles.label}>{translations["reject"]}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity activeOpacity={0.5} onPress={()=> confirmationAccept('accept')} style={[styles.btn, styles.btnApproved]}>
