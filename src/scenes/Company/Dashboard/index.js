...
import { Image, StyleSheet, View, StatusBar, Text, ScrollView, FlatList, PermissionsAndroid } from 'react-native';
import { BaseContainer, CardWasteBank, CardBannerDashboard, MyView, ButtonChats, FilterDate } from '@components';
...
function CompanyDashboard({ navigation, users, wasteBanks }) {
    ...
    const getDashboard = async (val) => {
        setDate(val)
        await wasteBanksUtils.getCompanyDashboard(`year=${moment(val).format('YYYY')}&month=${moment(val).format('M')}`)
    }
            ...
            <ScrollView style={{backgroundColor:'white'}}>
                <LinearGradient
                    ...
                    style={styles.header}>
                        ...
                        <Text style={styles.welcome} numberOfLines={2}>{`${translations["hello"]}, ${user?.organization?.companyName}, ${translations["how.are.you"]}`}</Text>
                    ...
                    {loadingWaste ? (
                        ...
                        <FlatList
                            ...
                            renderItem={({ item, index }) => (
                                index < 10 ?
                                <CardWasteBank item={item} index={index} onPress={()=> getResultWasteBanks(item)}/> : null
                        )}/>
                    )}
                </MyView>
                <CardBannerDashboard/>
            </ScrollView>
            <ButtonChats navigation={navigation} onPress={()=> navigation.navigate('Chats')}/>
        </BaseContainer>
    )
}
