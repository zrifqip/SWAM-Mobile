...
import { BaseContainer, AppBar, CardWasteBankProductList, FilterCategory, ButtonChats } from '@components';
...
function CompanyWasteBankDetail({ navigation, wasteBanks }) {
    ...
    const filter = selectCategory == 'Semua' ? product :  product.filter(x => x.category._id == selectCategory)
    ...
    return (
        <BaseContainer>
            ...
            <ScrollView>
                ...
                <Text style={[styles.txtTitle, StC.mB5, {marginLeft: RFValue(15)}]}>Stok</Text>
                ...
                <FlatList
                    data={filter}
                    renderItem={({ item }) => (
                        <CardWasteBankProductList item={item}/>
                    )}
                />
            </ScrollView>
