...
import { Image, StyleSheet, View, StatusBar, Text, ScrollView, FlatList, TouchableOpacity } from 'react-native';
import { BaseContainer, CardWasteBankProduct, MyView, CardTrasactionDashboardWasteBank, CardBannerDashboard, FilterDate } from '@components';
...
function WasteBankDashboard({ navigation, users, wasteBanks, transactions }) {
    ...
    const getSummary = async (val) => {
        ...
        await wasteBanksUtils.getSummary(`year=${moment(val).format('YYYY')}&month=${moment(val).format('M')}`)
            ...
            <ScrollView style={{backgroundColor:'white'}}>
                <LinearGradient
                    ...
                    style={styles.header}>
                        ...
                        <Text style={styles.nameBank} numberOfLines={2}>{`${user?.organization?.companyName}`}</Text>
                        <View style={[StC.flexR, {alignItems:'center'}]}>
                            <Image source={Icons.location} style={styles.iconPin} />
                            <Text style={styles.address} numberOfLines={1}>{`${user?.organization?.address?.street}`}</Text>
                        </View>
                </LinearGradient>
                <View style={styles.cardSaldo}>
                    <View style={styles.itemSaldo}>
                        <Text style={styles.saldo}>{numberFloat(transactions?.transactions?.length)}</Text>
                        <Text style={styles.labelSaldo}>{translations["transaction"]}</Text>
                    </View>
                    <View style={styles.itemSaldo}>
                        <Text style={styles.saldo}>{numberFloat(wasteBanks?.product?.length)}</Text>
                ...
                <View style={styles.summary}>
                    ...
                    {wasteBanks.summary.map((item, index) => (
                        <HStack>
                            <Icon as={Entypo} name={'dot-single'} size={RFValue(5)} color={Colors.GRAY_LABEL}/>
                    ...
                    {wasteBanks.summary.length == 0 && 
                        <Center>
                            <Text style={[styles.labelSaldo, StC.mB5]}>Tidak ada ringkasan</Text>
                        </Center>
                    }
                </View>
                    ...
                    {loadingProduct ? (
                        ...
                        <FlatList
                            ...
                            renderItem={({ item, index }) => (
                                <CardWasteBankProduct item={item} index={index} onPress={()=> getDetailProduct(item._id)}/>
                        )}/>
                    )}
                </MyView>
                <CardBannerDashboard/>
                <MyView hide={transactions.transactions.length == 0} style={styles.cardLine}>
                    ...
                    {loadingTransaction ? (
                        ...
                        <FlatList
                            ...
                            renderItem={({ item, index }) => (
                                index < 10 ?
                                <CardTrasactionDashboardWasteBank item={item} index={index} onPress={()=> getTransactionDetail(item._id)}/> : null
                        )}/>
                    )}
                </MyView>
            </ScrollView>
            <TouchableOpacity style={styles.btnChat} activeOpacity={0.5} onPress={()=> navigation.navigate('Chats')}>
