...
import { BaseContainer, AppBar, ButtonFlex, FormInputPhoto, FormInputCurrency, FormInput, FormInputPicker } from '@components';
...
function WasteBankProductForm({ navigation, wasteBanks }) {
    ...
    const [sellingPrice, setSellingPrice]   = useState(detail ? JSON.stringify(detail.sellingPrice) : '')
    const [purchasePrice, setPurchasePrice] = useState(detail ? JSON.stringify(detail.purchasePrice) : '')
    const [weight, setWeight]               = useState(detail ? JSON.stringify(detail.weight) : 0)
    ...
    const handleSave = async () => {
        ...
        if(purchasePrice && name){
            ...
            if(detail){
                ...
                if(photoUri){
                    await wasteBanksUtils.updatePhotoWasteBanksProduct(photoTemp, respons)
                }
    ...
    const _response = async (respons, type) => {
        if(respons == 400){
            showToast(type == 'save' ? translations["save.failed"] : translations["delete.failed"])
        } else {
            showToast(type == 'save' ? translations["save.success"] : translations["delete.success"])
            ...
            <ScrollView>
                <View style={styles.authCont}>
                    ...
                    <View style={[StC.flexR, StC.mT5]}>
                        <Text style={[StC.title, {flex:1}]}>{translations["info.industry"]}</Text>
                        <Switch isChecked={isSell} colorScheme="blue" onValueChange={(val)=> {setIsSell(val), AnimationLayout()}}/>
                    </View>
                </View>
            </ScrollView>
            <ButtonFlex
