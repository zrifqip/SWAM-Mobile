...
const KEYS_TO_FILTERS = ['nominal', 'status', 'createdAt', 'customerID.fullName'];
...
function History({ withdraw, _onRefresh }) {
    ...
    const arr = withdraw.withdraw.filter(x => x.status == 'accept' || x.status == 'reject')
    ...
    return (
        ...
        <FlatList
            data={filteredData}
            renderItem={(({ item }) => (
                <CardWithdrawWasteBank item={item}/>
            ))}
            ListEmptyComponent={
                <EmptyData message={translations["empty.withdraw"]}/>
            }  
            refreshControl={
                <RefreshControl onRefresh={_onRefresh}/>
            }
        />
        </>
    )
