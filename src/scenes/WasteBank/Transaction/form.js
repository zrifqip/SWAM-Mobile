...
import { StyleSheet, View, ActivityIndicator, Text, ScrollView, FlatList, TouchableOpacity, Image, Alert } from 'react-native';
import { BaseContainer, AppBar, FormInputPicker, ButtonFlex, ModalWasteSale, MyView } from '@components';
...
function WasteBankTransactionForm({ navigation, wasteBanks, users }) {
    ...
    const _handleSave = async () => {
        ...
        'Hai *' + nasabah?.fullName + '*,' + '\n\n' +
        'Berikut invoice untuk transaksi terbaru kamu' + '\n\n' +
        'Nama Nasabah : ' + nasabah?.fullName + '\n' + 
        'No. Telp : ' + nasabah?.phoneNumber + '\n' + 
        'Alamat : ' + nasabah?.address?.street + '\n' +
        ...
        const respons = await transactionsUtils.createTransactionsWasteBank(arr, photo)
    ...
    if(itemsActive?.length != 0){
        total = itemsActive.map(arr => arr.subtotal).reduce((acc, arr) => parseFloat(arr) + parseFloat(acc));
    }
    ...
    return (
        <BaseContainer loading={loading}>
            <AppBar navigation={navigation} title={translations["create.transaction"]}/>
            <ScrollView>
                            ...
                            <View style={styles.cardItem} activeOpacity={0.5}>
                                <View style={{flex:1}}>
                                    ...
                                    <View style={StC.flexR}>
                                        <Text style={styles.txtWeight}>{numberFloat(item.weight)} kg x {currencyFloat(item.price)}</Text>
                                        <Text style={styles.subtotal}>{currencyFloat(item.subtotal)}</Text>
                                    </View>
                                </View>
                                <TouchableOpacity onPress={()=> removeItems(item.itemID)} activeOpacity={0.5} style={styles.btnRemove}>
                                    <Icon as={<FontAwesome5 name={'trash'}/>} size={RFValue(4)} color={Colors.DANGER}/>
                                </TouchableOpacity>
                            </View>
                        ))}
                    />
                    {loadingItems && <ActivityIndicator size="large" color={Colors.PRIMARY}/>}
                ...
                <View style={styles.dropdown}>
                    <Text style={StC.title}>{translations["photo"]} <Text style={styles.optional}>(Opsional)</Text></Text>
                    <TouchableOpacity style={styles.cardImage} activeOpacity={0.5} onPress={()=> setModal()}>
                        {photo ?
                            <Image source={{uri: photoUri}} style={styles.image}/>
                        : 
                            <Icon as={<FontAwesome5 name={'camera'}/>} size={RFValue(8)} color={Colors.GRAY_SOFT}/>
                        }
                    </TouchableOpacity>
                </View>
            </ScrollView>
            <ButtonFlex
                ...
                style={{marginHorizontal: RFValue(15), marginBottom: RFValue(0), backgroundColor: Colors.BACKGROUND}}
            />
            <ButtonFlex
                disabled={!(users?.customers?.fullName && users?.customers?.fullName != '') || itemsActive.length == 0}
