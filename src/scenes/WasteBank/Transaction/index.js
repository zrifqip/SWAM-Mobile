...
import { BaseContainer, ButtonFab, CardTrasactionWasteBank, Search, EmptyData } from "@components";
...
const KEYS_TO_FILTERS = ['status', 'totalWeight', 'totalPrice', 'customer.fullName', 'date'];
...
function WasteBankTransaction ({ navigation, transactions }) {
    ...
    const exportDataToExcel = async () => {
        ...
        var file = RNFS.DownloadDirectoryPath + '/' + moment().format('DDMMYY-') + 'Transaction.xlsx';
    ...
    return (
        <BaseContainer loading={loading}>
            ...
            <FlatList
                ...
                renderItem={(({ item }) => (
                    <CardTrasactionWasteBank item={item} onPress={()=> getTransactionDetail(item._id)}/>
                ))}
                ListEmptyComponent={
                    <EmptyData message={translations["empty.transaction"]}/>
                }
                refreshControl={
                    <RefreshControl onRefresh={getTransaction}/>
                }
            />
            {transactions.transactions.length != 0 ? <ButtonFab label={'Download'} onPress={()=> handleClick()}/> : null}
        </BaseContainer>
    );
}
