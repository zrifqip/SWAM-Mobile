...
import { BaseContainer, CardWasteBankProductCategory, Search, EmptyData, AppBar, ButtonFab, ModalCategory } from '@components';
...
function ResultsCategory({ navigation, wasteBanks }) {
    ...
    const filteredData = category.filter(createFilter(searchTerm, KEYS_TO_FILTERS))
    ...
    return (
        <BaseContainer>
            ...
            <FlatList
                data={filteredData}
                renderItem={({ item }) => (
                    <CardWasteBankProductCategory item={item} onPress={()=> setCategory(item)} onPressEdit={()=> detailCategory(item)}/>
                )}
                ListEmptyComponent={
                    <EmptyData message={translations["empty.category"]}/>
                }  
            />
            <ButtonFab label={translations["add"]} onPress={()=> detailCategory(null)}/>
            <ModalCategory
