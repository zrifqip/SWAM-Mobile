...
import { currencyFloat, formatDateTime, numberFloat, arrStatus } from '@constants';
...
function UsersTransactionDetails({ navigation, transactions }) {
    ...
    return (
        <BaseContainer>
            <AppBar navigation={navigation} title={'Detail ' + translations["transaction"]}/>
            <ScrollView>
                <FlatList
                    data={arr}
                    renderItem={({ item }) => (
                        <View style={styles.card}>
                                ...
                                <View style={styles.flex}>
                                    {item.type == 'detail' ? 
                                        <>
                                            ...
                                            <Text style={[styles.txtValue, res.color && {color: res.color}]}>{res.value}</Text>
                                        </>
                                    : 
                                        <>
                                            <Text style={styles.txtName}>{res?.item?.name} {numberFloat(res?.weight)} kg</Text>
                                            <Text style={styles.txtValue}>{currencyFloat(res.totalPrice)}</Text>
                                        </>
                                    }
                                </View>
                            ))}
                            <MyView style={styles.flex} hide={item.type != 'item'}>
                                ...
                                <Text style={styles.txtValue}>{currencyFloat(details.totalPrice)}</Text>
                            </MyView>
                        </View>
                    )}
                />
                <View style={styles.card}>
                    ...
                    {image ?
                        <TouchableOpacity style={styles.cardImage} activeOpacity={0.5} onPress={()=> setIsVisible(true)}>
