...
import { Image, StyleSheet, View, StatusBar, Text, ScrollView, FlatList, PermissionsAndroid, TouchableOpacity, Alert } from 'react-native';
import { BaseContainer, CardWasteBank, CardTrasactionDashboard, MyView, CardBannerDashboard } from '@components';
            ...
            <ScrollView style={{backgroundColor:'white'}}>
                <LinearGradient
                    ...
                    style={styles.header}>
                        ...
                        <Text style={styles.welcome} numberOfLines={2}>{`${translations["hello"]}, ${user?.biodata?.fullName}, ${translations["how.are.you"]}`}</Text>
                        <View style={[StC.flexR, {alignItems:'center'}]}>
                            <Image source={Icons.location} style={styles.iconPin} />
                            <Text style={styles.address} numberOfLines={1}>{`${user?.biodata?.address?.street}`}</Text>
                        </View>
                </LinearGradient>
                <View style={styles.cardSaldo}>
                    <TouchableOpacity style={styles.itemSaldo} activeOpacity={0.5} onPress={()=> navigation.navigate('UsersWithdraw')}>
                        <Text style={styles.saldo}>{currencyFloat(user?.biodata?.balance)}</Text>
                    ...
                    {loadingWaste ? (
                        ...
                        <FlatList
                            ...
                            renderItem={({ item, index }) => (
                                index < 10 ?
                                <CardWasteBank item={item} index={index} onPress={()=> getResultWasteBanks(item._id)}/> : null
                        )}/>
                    )}
                </MyView>
                <CardBannerDashboard/>
                <MyView hide={history.length == 0} style={styles.cardLine}>
                    ...
                    {loadingTrans ? (
                        ...
                        <FlatList
                            ...
                            renderItem={({ item, index }) => (
                                index < 10 ?
                                <CardTrasactionDashboard item={item} index={index} onPress={()=> getTransactionDetail(item._id)}/> : null
                        )}/>
                    )}
                </MyView>
            </ScrollView>
