...
import { BaseContainer, AppBar, CardWasteBankProductList, ButtonFlex, FilterCategory } from '@components';
...
function UsersWasteBankDetail({ navigation, wasteBanks }) {
    ...
    const filter = selectCategory == 'Semua' ? wasteBanks?.items :  wasteBanks?.items.filter(x => x.category._id == selectCategory)

    return (
        <BaseContainer loading={loading}>
            <AppBar navigation={navigation} title={'Detail ' + translations["waste.banks"]}/>
            <ScrollView>
                ...
                <Text style={[styles.txtTitle, StC.mB5, {marginLeft: RFValue(15)}]}>{translations["product"]}</Text>
                <FilterCategory selected={selectCategory} data={filterCategory(wasteBanks?.items)} onPress={(key)=> setSelectCategory(key)}/>
                <FlatList
                    data={filter}
                    renderItem={({ item }) => (
