...
function UsersWithdraw ({ navigation }) {
    ...
    const RunningRoute = () => (
        <Running navigation={navigation} onPress={(id)=> getTransactionDetail(id)} _onRefresh={()=> getTransaction()}/>
    );

    const HistoryRoute = () => (
        <History navigation={navigation} onPress={(id)=> getTransactionDetail(id)} _onRefresh={()=> getTransaction()}/>
    );
    ...
    return (
        <BaseContainer loading={loading}>
            ...
            />
            <ButtonFab label={translations["withdraw"]} onPress={()=> refModalWithdraw.current.open()}/>
            <ModalWithdraw
