...
import { FlatList, StyleSheet, View, Linking, Text, ScrollView, TouchableOpacity, Alert } from 'react-native';
...
function Account({ navigation, users }) {
    ...
    let role                            = user.role
    let biodata, organization, photo    = ''
            ...
            submenu:[
                {
                    ...
                    action: role == 'user' ? 'UsersProfile' : role == 'bank-sampah' ? 'WasteBankProfile' : 'CompanyProfile'
                },
                {
                    icon: MaterialCommunityIcons,
                    name: 'wallet',
                    menu: translations["min.withdraw"],
                    value: role == 'bank-sampah' ? currencyFloat(organization.minimumWithdrawal) : '',
                    type: 'modal',
                    action: 'minwithdrawl',
                ...
                },
                {
                    ...
                    name: 'wallet',
                    menu: 'Bank Transfer',
                    value: role == 'user' ? user?.biodata?.bankAccount ? user?.biodata?.bankAccount?.accountName : '' : '',
                    type: 'modal',
                    action: 'bankTransfer',
                    disabled: role != 'user'
                },
        ...
        <BaseContainer>
            <ScrollView>
                <View style={styles.header}>
                    <Avatar square size="lg" source={photo ? { uri: base_uri + photo } : Images.profile} />
                    <View style={styles.contentHeader}>
                        {role == 'bank-sampah' ? 
                        <Text style={[styles.name, {color: Colors.PRIMARY}]}>Bank Sampah</Text> : null}
                        <Text style={styles.waste}>
                            {role == 'user' ? user?.biodata?.fullName : role == 'bank-sampah' || role == 'pengepul' ? user?.organization?.companyName : null}</Text>
                        <Text style={styles.street}>{role == 'user' ? biodata?.address?.street : user?.organization?.address?.street}</Text>
                    </View>
                </View>
                <FlatList
                    data={menu}
                    renderItem={(({ item }) => (
                        <View style={[styles.cardMenu, {paddingTop: RFValue(item.type == 'logout' ? 0 : 10)}]}>
                            {item.type != 'logout' ? <Text style={styles.name}>{item.menu}</Text> : null}
                            {item.submenu.filter(x => !x.disabled).map((submenu) => (
                                <TouchableOpacity style={styles.submenu} onPress={()=> navigationMenu(submenu)}>
                                    <View style={styles.icon}>
                                        <Icon as={submenu.icon} name={submenu.name} size={RFValue(5)} color={[item.type == 'logout' ? Colors.PRIMARY : Colors.BLACK]}/>
                                    </View>
